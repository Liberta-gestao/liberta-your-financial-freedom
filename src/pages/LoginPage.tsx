...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Não foi possível entrar", { description: String((err as any)?.message ?? err) });
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm space-y-8">
        <div className="text-center">
          ...
          <p className="text-muted-foreground text-sm mt-1">Entre na sua conta Liberta</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input id="password" type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="h-11 bg-secondary border-border" />
          </div>
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              toast.error("Não foi possível entrar com Google", { description: String((err as any)?.message ?? err) });
              setBusy(false);
            }
          }}
        >
        ...
        <p className="text-center text-sm text-muted-foreground">
          Não tem conta?{" "}
          <button className="text-primary hover:underline font-medium" onClick={() => navigate("/signup")}>Criar conta grátis</button>
        </p>
      </div>
    </div>
  );
